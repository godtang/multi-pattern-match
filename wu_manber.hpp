...
namespace wu_manber
{
  ...
  public:
    ...
    WuManber(unsigned short int HBITS = 4, std::size_t tableSize = 32768) : isInitialized_(false), m_(0), k_(0),
    ...
    void scan(const StringType &text, std::map<std::string, int> &record, std::function<void(const StringType &, const StringType &, std::size_t, std::size_t, std::map<std::string, int> &)> onMatch)
    {
      ...
      if (isShortPatternExist_)
      {
        ...
        {
          onMatch(text, lengthOnePatterns_[firstCharacterMatchIndex], firstCharacterMatchIndex, 0, record);
        }
        ...
        for (std::size_t idx = 1; idx < PRE_WU_MANBER_LIMIT; ++idx)
        {
          ...
          checkShortPattern_(text, record, idx, onMatch);
        }
      }
        ...
        if (shiftLength == 0)
        {
          ...
          for (const auto &potentialMatch : hashPrefixTable_[hashValue])
          {
            if (prefixHash == potentialMatch.prefixHash)
            {
              ...
              {
                onMatch(text, pattern, potentialMatch.idx, idx - m_ + 1, record);
              }
            }
          }
        }
  ...
  private:
    ...
    void checkShortPattern_(const StringType &text, std::map<std::string, int> &record, std::size_t cur_idx, std::function<void(const StringType &, const StringType &, std::size_t, std::size_t, std::map<std::string, int> &)> onMatch) const
    {
      ...
      if (l1MatchIndex > -1)
      {
        onMatch(text, lengthOnePatterns_[l1MatchIndex], l1MatchIndex, cur_idx, record);
      }

      int l2MatchIndex = lengthTwoPatternLookup_[static_cast<std::size_t>(text[cur_idx - 1])][static_cast<std::size_t>(text[cur_idx])];
      if (l2MatchIndex > -1)
      {
        onMatch(text, lengthTwoPatterns_[l2MatchIndex], l2MatchIndex, cur_idx - 1, record);
      }
    }
